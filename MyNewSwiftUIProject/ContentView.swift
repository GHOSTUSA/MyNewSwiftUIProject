...
class Inventory: ObservableObject {
    @Published var loot: [LootItem]
      ...
      init() {
          // Initialiser avec des objets spécifiques à des types d'armes
          loot = [
            // Halo
...
struct ContentView: View {
    ...
    var body: some View {
        NavigationStack {
            List {
                Button(action: {
                    let newItem = LootItem(name: "Épée du Dragon", type: .fire, rarity: .legendary, attackStrength: 50, game: .emptyGame)
